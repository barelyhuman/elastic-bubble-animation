(function (global, factory) {
    ...
    ElasticBubbleAnimation.prototype.init = function init () {
        ...
        this.opts.el.addEventListener('mouseenter', function (e) {
            this$1.opts.el.addEventListener('mousemove', this$1.mouseMoveHandler);
        });


        this.opts.el.addEventListener('mouseleave', function (e) {
            this$1.opts.el.removeEventListener('mousemove', this$1.mouseMoveHandler);
            setTimeout(function () {
    ...
    ElasticBubbleAnimation.prototype.addBubble = function addBubble () {
        ...
        var rgba = this.hexToRgbA(this.colors[this.getRandomNumber(0, this.colors.length)], 0.5);
    ...
    ElasticBubbleAnimation.prototype.moveRandomly = function moveRandomly (xBounds, yBounds, bubble, mousePosition) {
        var xRandomPoint = this.getRandomNumber(Math.min(xBounds[0], xBounds[1]), Math.max(xBounds[0], xBounds[1]));
        var yRandomPoint = this.getRandomNumber(Math.min(yBounds[0], yBounds[1]), Math.max(yBounds[0], yBounds[1]));
    ...
    ElasticBubbleAnimation.prototype.resetBubblePositions = function resetBubblePositions (event) {
        ...
        this.bubbles.forEach(function (bubble) {
            ...
            if (this$1.initialPosition[bubble.id]) {
                bubble.bubble.style.top = this$1.initialPosition[bubble.id].y + 'px';
                bubble.bubble.style.left = this$1.initialPosition[bubble.id].x + 'px';
            }
